...
export async function POST(request: Request) {
  try {
    ...
    if (user.isApproved) {
      return NextResponse.json(
        { error: "Utilisateur déjà approuvé" },
        { status: 400 }
      );
    }
    ...
    // Contenu de l'email
    const mailOptions = {
      from: process.env.SMTP_FROM || "norereply@centreculturiel.cd",
      to: user.email,
    ...
    // Rediriger vers la page des approbations
    return NextResponse.redirect(
      new URL("/espace-membre/admin/approvals?success=true", request.url)
    );
  } catch (error) {
